...
all:*.go
	...
	env GOOS=darwin GOARCH=arm64 go build -ldflags "-s -w -X main.version=$(VERSION) -X main.compileDate=$(COMPILEDATE)" -o check_abuseipdb-darwin-arm64
	env GOOS=linux GOARCH=arm64 go build -ldflags "-s -w -X main.version=$(VERSION) -X main.compileDate=$(COMPILEDATE)" -o check_abuseipdb-linux-arm64
